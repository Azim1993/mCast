...
const contentLength = computed(() => {
    return {
        remaining: 255 - timeline.value.content.length,
        isShow: timeline.value.content.length > 100,
        bgClass: timeline.value.content.length < 201 ? 'bg-green-600' : (
            timeline.value.content.length < 256 ? 'bg-yellow-600' : 'bg-red-600'
        ) 
    }
})
...
<template>
    ...
    <form method="post" @submit.prevent="handleSubmit" class="mb-4 text-center">
        <textarea v-model="timeline.content" class="p-4 h-24 w-full border border-gray-200 rounded-t focus:outline-secondary-light" placeholder="What is happening Now?"></textarea>
        <div class="border -mt-2 border-gray-200 border-t-0 py-2 pt-3 px-5 w-full text-center rounded-b-lg">
            <div class="flex justify-start">
                ...
                <p v-if="contentLength.isShow" class="px-2 py-1 rounded-lg text-white" :class="contentLength.bgClass">
                    Remaining: {{ contentLength.remaining }}
                </p>
            </div>
